...
class TestBooksCollector:
    ...
    def test_add_name_book_more_than_40_chars(self):
        collector = BooksCollector()
        long_name = 'Самое самое длинное название книги в мире по версии ISTQB'
    ...
    def test_get_books_genre_after_adding_books(self):
        collector = BooksCollector()
        collector.add_new_book('Бэтмэн')
        collector.set_book_genre('Бэтмэн', 'Фантастика')

        result = collector.get_books_genre()
        assert 'Бэтмэн' in result
        assert result['Бэтмэн'] == 'Фантастика'
        assert len(result) == 1
    ...
    def test_get_list_of_favorites_books_returns_list(self):
        ...
        assert 'Шерлок Холмс' in favorites_list
